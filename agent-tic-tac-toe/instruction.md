...
3. Read the board from `wait_for_turn.ts` output and decide your move.

4. Make your move:

```bash
node move.ts <position> --name "Agent"
```

5. Go back to step 2 and repeat until `wait_for_turn.ts` exits with code `1`.

## Rules

- Always wait before moving.
- Never make two moves in a row.
- If `move.ts` fails, read the error and try again.
