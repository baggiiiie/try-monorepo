...
const DEFAULT_EMAIL_COUNT = 3
const DEFAULT_READ_DELAY_MS = 1500
const DEFAULT_BODY_MAX_CHARS = 4000
...
export async function runOutlookCheck({
  ...
  const emailCheck = emailCount > 0
    ? await readTopInboxEmails(gui, { emailCount, maxNodes, readDelayMs, bodyMaxChars })
    : skippedEmailCheck()
...
async function readTopInboxEmails(gui, { emailCount, maxNodes, readDelayMs, bodyMaxChars }) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    const content = readReadingPane(gui.scope, {
      maxNodes,
  ...
  return {
    ...
    selectionMethod: 'mouse-click-row-center',
    readDelayMs,
    bodyMaxChars,
...
function clickNodeCenter(node) {
  ...
  if (!center) return { success: false, message: 'Cannot select message row without a bounding box' }

  const point = { x: Math.round(center.x), y: Math.round(center.y) }
  const mouse = new MouseController()
  mouse.moveMouse(point.x, point.y, Coordinate.Abs)
  mouse.button(Button.Left, Direction.Click)
  return { success: true, method: 'mouse-click-row-center', point }
}
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  runOutlookCheck()
    .then((report) => {
      ...
      if (report.results.some((result) => !result.success) || !report.emailCheck.success) process.exitCode = 1
    })
    .catch((error) => {
