...
def parse_add_command(text: str) -> ParsedExpense | None:
    ...
    participants_text = re.sub(r"\s+and\s+", ", ", with_match.group(1), flags=re.IGNORECASE)
    participants = [p.strip().lower() for p in participants_text.split(",") if p.strip()]
    ...
    return ParsedExpense(title=title, amount=amount, paid_by=paid_by.lower(), participants=participants)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data.startswith("yes_"):
        ...
        try:
            spliit.add_expense(title=title, paid_by=paid_by_id, paid_for=paid_for, amount=amount)
