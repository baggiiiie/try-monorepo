...
- `POST /api/sync/push`, `GET /api/sync/pull?since=` — sync endpoints
...
| `SyncService` | Push/pull logic — timestamps, conflict resolution, server-as-truth |
...
| **Services** | `SyncService` — push/pull to Go server |

### Sync Flow

1. **Push**: iOS collects records with `syncStatus == "pending_push"`, POSTs to `/api/sync/push`. Server responds with canonical IDs and timestamps. iOS updates local records to `synced`.
2. **Pull**: iOS GETs `/api/sync/pull?since=<lastPullAt>`. Server returns all records modified since that timestamp. iOS upserts them locally (server wins on conflict).

---

## E2E Tests — `tests/e2e/`

8 bash scripts that spin up the server and exercise it via HTTP:
...
| `08_sync_equal_timestamp_update.sh` | Timestamp tie-breaking |
...
- **Server is source of truth** — timestamp-based, last-edit-wins conflict resolution
