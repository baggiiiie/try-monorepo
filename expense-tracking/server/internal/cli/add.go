...
func newAddCmd(tx txRunnerProvider, expenses expenseServiceProvider, prefs preferencesServiceProvider) *cobra.Command {
	return BulkCommand[AddInput, *service.Expense]{
		Use:   "add",
...
For recurring expenses, see 'expense add-recurring'.`,
		...
		Tx:             tx,
		Process: func(ctx context.Context, q *dbsqlc.Queries, in AddInput) (*service.Expense, error) {
			prefService := prefs()
			if prefService == nil {
			...
			if expenseService == nil {
				return nil, fmt.Errorf("expense service is not initialized")
			}
			...
			return expenseService.CreateWithQueries(ctx, q, service.ExpenseInput{
		...
		FormatHumanRow: func(w io.Writer, _ int, exp *service.Expense) {
