...
const listExpensesByDateWindow = `-- name: ListExpensesByDateWindow :many
SELECT e.id, e.amount, e.currency, e.category_id, e.description, e.merchant, e.date, e.source, e.created_at, e.updated_at, e.deleted_at, e.server_version, e.client_updated_at, c.name AS category_name
FROM expenses e
LEFT JOIN categories c ON e.category_id = c.id
WHERE e.deleted_at IS NULL
...
type ListExpensesByDateWindowParams struct {
	Before  int64 `json:"before"`
	Since   int64 `json:"since"`
	MaxRows int64 `json:"max_rows"`
}

type ListExpensesByDateWindowRow struct {
	ID              string         `json:"id"`
	Amount          int64          `json:"amount"`
	Currency        string         `json:"currency"`
	CategoryID      string         `json:"category_id"`
...
func (q *Queries) ListExpensesByDateWindow(ctx context.Context, arg ListExpensesByDateWindowParams) ([]ListExpensesByDateWindowRow, error) {
	rows, err := q.db.QueryContext(ctx, listExpensesByDateWindow, arg.Before, arg.Since, arg.MaxRows)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListExpensesByDateWindowRow
	for rows.Next() {
		var i ListExpensesByDateWindowRow
		if err := rows.Scan(
			&i.ID,
			&i.Amount,
			&i.Currency,
